...
function HolidayThemeList() {
  ...
  const fetchThemes = async () => {
    try {
      const res = await axios.get(
        `/api/holiday-themes${search ? "?search=" + search : ""}`
      );
      setThemes(res.data);
    } catch (err) {
      console.error(err);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editId) {
        await axios.put(`/api/holiday-themes/${editId}`, fd);
      } else {
        await axios.post("/api/holiday-themes", fd);
      }
  ...
  const handleEdit = (item) => {
    ...
    setForm({
      ...
      themeImage: null, // Keep null so Cloudinary doesn't get old URL
    });
    ...
    <div className="theme-container">
      ...
      {showForm && (
        <form className="theme-form" onSubmit={handleSubmit}>
          ...
          <h3>{editId ? "Update Theme" : "Add Theme"}</h3>
          ...
          <input
            ...
            onChange={(e) =>
              setForm({ ...form, themeSlug: e.target.value })
            }
          />
          ...
          <select
            ...
            onChange={(e) =>
              setForm({ ...form, showOnHome: e.target.value === "yes" })
            }
          >
          ...
          <input
            type="file"
            onChange={(e) =>
              setForm({ ...form, themeImage: e.target.files[0] })
            }
          />
