...
// const API = "http://localhost:9000/api/hotels";
...
//     setRooms([
//       ...rooms,
//       { roomType: "", basePrice: "", totalRooms: "", availableRooms: "" }
//     ]);
//   };

//   const handleRoomChange = (i, e) => {
//     const newRooms = [...rooms];
//     newRooms[i][e.target.name] = e.target.value;
//     setRooms(newRooms);
//   };
...
//     // images
//     if (form.hotelImages) {
//       for (let img of form.hotelImages) {
//         fd.append("hotelImages", img);
//       }
//     }
...
//       {/* ================= FORM ================= */}
//       <form onSubmit={handleSubmit} style={{ border:"1px solid #ccc",padding:20,marginBottom:40 }}>
        
//         <input name="hotelName" placeholder="Hotel Name" onChange={handleChange} /><br/>
...
//         <input name="pricePerNight" placeholder="Price" onChange={handleChange} /><br/>
//         <input name="vendorId" placeholder="Vendor Id" onChange={handleChange} /><br/>
//         <input name="vendorName" placeholder="Vendor Name" onChange={handleChange} /><br/>
...
//         {/* Images */}
//         <input type="file" multiple onChange={(e)=>setForm({...form,hotelImages:e.target.files})}/>
...
//         {rooms.map((r,i)=>(
//           <div key={i}>
//             <input name="roomType" placeholder="Room Type" onChange={(e)=>handleRoomChange(i,e)}/>
//             <input name="basePrice" placeholder="Price" onChange={(e)=>handleRoomChange(i,e)}/>
//             <input name="totalRooms" placeholder="Total Rooms" onChange={(e)=>handleRoomChange(i,e)}/>
//             <input name="availableRooms" placeholder="Available" onChange={(e)=>handleRoomChange(i,e)}/>
//           </div>
//         ))}
...
//               <td>{h.vendorName}</td>
//               <td>
//                 <button onClick={()=>viewHotel(h)}>View</button>
...
//       {/* ================= VIEW ALL DATA ================= */}
//       {viewData && (
//         <div style={{marginTop:40,background:"#000",color:"#0f0",padding:20}}>
//           <h2>Full Hotel Data</h2>
...
//     /* MAIN */
//     .hm-main { padding: 28px 32px; }

//     /* HEADER */
//     .hm-header { display: flex; align-items: center; justify-content: space-between; margin-bottom: 28px; }
//     .hm-header-left h2 { font-family: 'Syne', sans-serif; font-size: 24px; font-weight: 800; letter-spacing: -.5px; }
//     .hm-header-left p { font-size: 13px; color: var(--muted); margin-top: 2px; }
//     .hm-btn-add {
//       display: flex; align-items: center; gap: 8px; background: var(--accent);
//       color: #000; border: none; padding: 11px 20px; border-radius: var(--r);
//       font-family: 'Syne', sans-serif; font-weight: 700; font-size: 13px;
//       cursor: pointer; transition: .2s; letter-spacing: .3px;
//     }
//     .hm-btn-add:hover { background: var(--accent2); transform: translateY(-1px); }
//     .hm-btn-add svg { width: 15px; height: 15px; }

//     /* STATS */
//     .hm-stats { display: grid; grid-template-columns: repeat(4, 1fr); gap: 14px; margin-bottom: 24px; }
//     .hm-stat {
//       background: var(--surf); border: 1px solid var(--border);
//       border-radius: var(--r); padding: 18px 20px;
//     }
//     .hm-stat .lbl { font-size: 10px; text-transform: uppercase; letter-spacing: .8px; color: var(--muted); margin-bottom: 8px; }
//     .hm-stat .val { font-family: 'Syne', sans-serif; font-size: 30px; font-weight: 800; }
//     .hm-stat .bdg { display: inline-block; font-size: 11px; padding: 3px 9px; border-radius: 20px; margin-top: 6px; font-weight: 500; }
//     .bdg-blue { background: rgba(59,130,246,.12); color: var(--blue); }
//     .bdg-green { background: rgba(16,185,129,.12); color: var(--green); }
//     .bdg-amber { background: rgba(245,158,11,.12); color: var(--accent); }
//     .bdg-red { background: rgba(239,68,68,.12); color: var(--red); }

//     /* TABLE CARD */
//     .hm-table-card { background: var(--surf); border: 1px solid var(--border); border-radius: var(--r); overflow: hidden; }
//     .hm-table-top {
//       display: flex; align-items: center; justify-content: space-between;
//       padding: 18px 22px; border-bottom: 1px solid var(--border);
//     }
//     .hm-table-top h3 { font-family: 'Syne', sans-serif; font-weight: 700; font-size: 14px; }
//     .hm-search {
//       display: flex; align-items: center; gap: 8px; background: var(--surf2);
//       border: 1px solid var(--border); border-radius: 8px; padding: 8px 13px; width: 220px;
//     }
...
//       text-transform: uppercase; letter-spacing: .8px; color: var(--muted); font-weight: 500;
//     }
//     .hm-table td { padding: 13px 14px; font-size: 13px; border-top: 1px solid var(--border); vertical-align: middle; }
//     .hm-table tbody tr:hover td { background: rgba(255,255,255,.015); }
//     .hm-hotel-name { font-weight: 500; }
//     .hm-hotel-sub { font-size: 11px; color: var(--muted); margin-top: 1px; }
//     .hm-rating { display: flex; align-items: center; gap: 3px; }
//     .hm-rating svg { width: 12px; height: 12px; fill: var(--accent); color: var(--accent); }
//     .hm-status {
...
//     .hm-status-approved { background: rgba(16,185,129,.12); color: var(--green); }
//     .hm-status-pending { background: rgba(245,158,11,.12); color: var(--accent); }
//     .hm-status-rejected { background: rgba(239,68,68,.12); color: var(--red); }
...
//     .hm-icon-btn.view:hover { border-color: var(--blue); color: var(--blue); background: rgba(59,130,246,.1); }
//     .hm-icon-btn.edit:hover { border-color: var(--accent); color: var(--accent); background: rgba(245,158,11,.1); }
//     .hm-icon-btn.del:hover { border-color: var(--red); color: var(--red); background: rgba(239,68,68,.1); }
...
//     /* SLIDE PANEL */
//     .hm-panel {
//       background: var(--surf); width: 800px; height: 100vh; overflow-y: auto;
...
//     }
//     .hm-panel-head h3 { font-family: 'Syne', sans-serif; font-size: 17px; font-weight: 800; }
//     .hm-panel-head p { font-size: 12px; color: var(--muted); margin-top: 2px; }
//     .hm-close {
...
//     /* FORM */
//     .hm-form-body { padding: 24px 26px; flex: 1; }
//     .hm-section-title {
...
//     }
//     .hm-section-title::after { content: ''; flex: 1; height: 1px; background: var(--border); }
//     .hm-section-title:first-child { margin-top: 0; }
//     .hm-g2 { display: grid; grid-template-columns: 1fr 1fr; gap: 14px; margin-bottom: 14px; }
//     .hm-g3 { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 14px; margin-bottom: 14px; }
//     .hm-g4 { display: grid; grid-template-columns: 1fr 1fr 1fr 1fr; gap: 14px; margin-bottom: 14px; }
//     .hm-fg { display: flex; flex-direction: column; gap: 5px; margin-bottom: 14px; }
//     .hm-fg label { font-size: 11px; color: var(--muted); font-weight: 500; letter-spacing: .3px; }
...
//       border-color: var(--accent); box-shadow: 0 0 0 3px rgba(245,158,11,.08);
//     }
//     .hm-fg select option { background: var(--surf2); }
//     .hm-fg textarea { resize: vertical; min-height: 72px; }
//     .hm-cb-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 8px; margin-bottom: 14px; }
//     .hm-cb-item {
//       display: flex; align-items: center; gap: 8px; background: var(--surf2);
...
//     .hm-cb-item label { font-size: 12px; cursor: pointer; user-select: none; }
//     .hm-file-wrap {
//       background: var(--surf2); border: 2px dashed var(--border); border-radius: 8px;
//       padding: 18px; text-align: center; cursor: pointer; transition: .2s; position: relative;
//     }
...
//     .hm-form-actions {
//       display: flex; gap: 10px; padding: 18px 26px; border-top: 1px solid var(--border);
...
//     }
//     .hm-btn-submit:hover { background: var(--accent2); }
//     .hm-btn-cancel {
//       background: var(--surf2); color: var(--muted); border: 1px solid var(--border);
//       padding: 12px 22px; border-radius: var(--r); font-family: 'DM Sans', sans-serif;
//       font-size: 14px; cursor: pointer; transition: .2s;
//     }
...
//     }
//     .hm-room-card-head { display: flex; align-items: center; justify-content: space-between; margin-bottom: 14px; }
//     .hm-room-card-head span { font-family: 'Syne', sans-serif; font-size: 13px; font-weight: 700; color: var(--accent); }
//     .hm-btn-rm-room {
//       background: rgba(239,68,68,.12); color: var(--red); border: none;
//       padding: 4px 10px; border-radius: 6px; font-size: 11px; cursor: pointer;
//       font-family: 'DM Sans', sans-serif; transition: .2s;
//     }
//     .hm-btn-rm-room:hover { background: rgba(239,68,68,.2); }
//     .hm-btn-add-room {
...
//     }
//     .hm-btn-add-room:hover { background: rgba(245,158,11,.2); }
...
//       width: 940px; max-height: 88vh; overflow-y: auto;
...
//       display: flex; align-items: center; justify-content: space-between;
//       padding: 22px 26px; border-bottom: 1px solid var(--border);
//       position: sticky; top: 0; background: var(--surf); border-radius: 14px 14px 0 0; z-index: 5;
//     }
//     .hm-view-head h3 { font-family: 'Syne', sans-serif; font-size: 19px; font-weight: 800; }
//     .hm-view-body { padding: 24px 26px; }
...
//     .hm-view-field { background: var(--surf2); border-radius: 8px; padding: 11px 12px; }
//     .hm-view-field label { font-size: 10px; text-transform: uppercase; letter-spacing: .7px; color: var(--muted); display: block; margin-bottom: 4px; }
//     .hm-view-field span { font-size: 13px; color: var(--text); font-weight: 500; }
//     .hm-yes { color: var(--green) !important; }
//     .hm-no { color: var(--red) !important; }
//     .hm-tags { display: flex; flex-wrap: wrap; gap: 6px; }
//     .hm-tag { background: var(--accent-dim); color: var(--accent); font-size: 11px; padding: 4px 10px; border-radius: 20px; }
...
//     /* TOAST */
//     .hm-toast {
//       position: fixed; bottom: 22px; right: 22px; padding: 12px 20px; border-radius: 9px;
...
//     .hm-toast.success { background: var(--green); color: #000; }
//     .hm-toast.error { background: var(--red); color: #fff; }

//     /* SCROLLBAR */
//     .hm-panel::-webkit-scrollbar, .hm-view-modal::-webkit-scrollbar, .hm-main::-webkit-scrollbar { width: 5px; }
//     .hm-panel::-webkit-scrollbar-track, .hm-view-modal::-webkit-scrollbar-track, .hm-main::-webkit-scrollbar-track { background: var(--surf); }
//     .hm-panel::-webkit-scrollbar-thumb, .hm-view-modal::-webkit-scrollbar-thumb, .hm-main::-webkit-scrollbar-thumb { background: var(--border2); border-radius: 3px; }
//   `;
...
// // ─── CONSTANTS ───────────────────────────────────────────────────────────────
// const AMENITY_LIST = [
//   { key: "wifi", label: "Free WiFi" }, { key: "pool", label: "Swimming Pool" },
//   { key: "gym", label: "Gym / Fitness" }, { key: "spa", label: "Spa & Wellness" },
//   { key: "restaurant", label: "Restaurant" }, { key: "bar", label: "Bar / Lounge" },
//   { key: "parking", label: "Parking" }, { key: "ac", label: "Air Conditioning" },
//   { key: "laundry", label: "Laundry" }, { key: "concierge", label: "Concierge" },
//   { key: "roomservice", label: "Room Service" }, { key: "lift", label: "Elevator / Lift" },
// ];

// const PM_LIST = [
//   { key: "cash", label: "Cash" }, { key: "card", label: "Credit/Debit Card" },
//   { key: "upi", label: "UPI" }, { key: "netbanking", label: "Net Banking" },
//   { key: "wallet", label: "Wallets" }, { key: "emi", label: "EMI" },
// ];

// const ROOM_BOOL_FIELDS = [
//   { key: "extraBedAvailable", label: "Extra Bed Available" }, { key: "balcony", label: "Balcony" },
//   { key: "airCondition", label: "Air Conditioning" }, { key: "heater", label: "Heater" },
//   { key: "wifi", label: "WiFi" }, { key: "tv", label: "TV / Smart TV" },
//   { key: "minibar", label: "Minibar" }, { key: "wardrobe", label: "Wardrobe" },
//   { key: "workDesk", label: "Work Desk" }, { key: "iron", label: "Iron" },
//   { key: "kitchen", label: "Kitchen" }, { key: "bathtub", label: "Bathtub" },
//   { key: "shower", label: "Shower" }, { key: "toiletries", label: "Toiletries" },
//   { key: "hairDryer", label: "Hair Dryer" }, { key: "breakfastIncluded", label: "Breakfast Incl." },
//   { key: "lunchIncluded", label: "Lunch Incl." }, { key: "dinnerIncluded", label: "Dinner Incl." },
//   { key: "smokingAllowed", label: "Smoking Allowed" }, { key: "refundable", label: "Refundable" },
// ];
...
//   return <div className="hm-fg"><label>{label}</label>{children}</div>;
// }
...
// function CbGrid({ items, checked, onChange }) {
//   return (
//     <div className="hm-cb-grid">
//       {items.map(({ key, label }) => (
//         <div key={key} className="hm-cb-item" onClick={() => onChange(key, !checked[key])}>
...
//   const upd = (field, val) => onChange(room.id, field, val);
//   return (
//     <div className="hm-room-card">
//       <div className="hm-room-card-head">
//         <span>Room Type #{index + 1}</span>
//         <button type="button" className="hm-btn-rm-room" onClick={() => onRemove(room.id)}>Remove</button>
//       </div>
//       <div className="hm-g3">
//         <Input label="Room Type *" value={room.roomType} onChange={v => upd("roomType", v)} placeholder="Deluxe / Suite..." />
//         <Input label="Room Name" value={room.roomName} onChange={v => upd("roomName", v)} placeholder="Ocean Deluxe" />
//         <Input label="Description" value={room.description} onChange={v => upd("description", v)} placeholder="Short desc..." />
//       </div>
//       <div className="hm-g3">
//         <Input label="Base Price (₹) *" type="number" value={room.basePrice} onChange={v => upd("basePrice", v)} placeholder="2000" />
//         <Input label="Offer Price (₹)" type="number" value={room.offerPrice} onChange={v => upd("offerPrice", v)} placeholder="1800" />
//         <Input label="Tax (%)" type="number" value={room.tax} onChange={v => upd("tax", v)} placeholder="12" />
//       </div>
//       <div className="hm-g3">
//         <Input label="Total Rooms *" type="number" value={room.totalRooms} onChange={v => upd("totalRooms", v)} placeholder="10" />
//         <Input label="Available Rooms *" type="number" value={room.availableRooms} onChange={v => upd("availableRooms", v)} placeholder="8" />
//         <Input label="Max Guests" type="number" value={room.maxGuests} onChange={v => upd("maxGuests", v)} placeholder="4" />
//       </div>
//       <div className="hm-g3">
//         <Input label="Adults" type="number" value={room.adults} onChange={v => upd("adults", v)} placeholder="2" />
//         <Input label="Children" type="number" value={room.children} onChange={v => upd("children", v)} placeholder="2" />
//         <Input label="Room Size" value={room.roomSize} onChange={v => upd("roomSize", v)} placeholder="35 sqm" />
//       </div>
//       <div className="hm-g3">
//         <Input label="Floor Number" value={room.floorNumber} onChange={v => upd("floorNumber", v)} placeholder="3" />
//         <Select label="Bed Type" value={room.bedType} onChange={v => upd("bedType", v)} options={["Single", "Twin", "Double", "King", "Queen", "Super King"]} />
//         <Input label="Bed Count" type="number" value={room.bedCount} onChange={v => upd("bedCount", v)} placeholder="1" />
//       </div>
//       <div className="hm-g3">
//         <Input label="View Type" value={room.viewType} onChange={v => upd("viewType", v)} placeholder="Sea View, Garden, City" />
//         <Input label="Bathroom Type" value={room.bathroomType} onChange={v => upd("bathroomType", v)} placeholder="Private / Shared" />
//         <Input label="Extra Bed Charge (₹)" type="number" value={room.extraBedCharge} onChange={v => upd("extraBedCharge", v)} placeholder="500" />
//       </div>
//       <div className="hm-g2">
//         <Input label="Cancellation Policy" value={room.cancellationPolicy} onChange={v => upd("cancellationPolicy", v)} placeholder="Free cancellation 24hr before..." />
//         <Select label="Room Status" value={room.status} onChange={v => upd("status", v)} options={[{ value: "available", label: "Available" }, { value: "soldout", label: "Sold Out" }]} />
//       </div>
//       <CbGrid items={ROOM_BOOL_FIELDS} checked={room} onChange={(key, val) => upd(key, val)} />
//     </div>
//   );
// }

// // ─── HOTEL FORM ──────────────────────────────────────────────────────────────
// function HotelForm({ form, setForm, onSubmit, onClose, isEdit }) {
//   const upd = (field, val) => setForm(f => ({ ...f, [field]: val }));
//   const addRoom = () => setForm(f => ({ ...f, rooms: [...f.rooms, defaultRoom()] }));
//   const removeRoom = id => setForm(f => ({ ...f, rooms: f.rooms.filter(r => r.id !== id) }));
//   const updateRoom = (id, field, val) => setForm(f => ({
//     ...f, rooms: f.rooms.map(r => r.id === id ? { ...r, [field]: val } : r)
//   }));
...
//         {/* BASIC INFO */}
//         <div className="hm-section-title">Basic Information</div>
//         <div className="hm-g2">
//           <Input label="Hotel Name *" value={form.hotelName} onChange={v => upd("hotelName", v)} placeholder="Grand Palace Hotel" />
//           <Select label="Hotel Type" value={form.hotelType} onChange={v => upd("hotelType", v)}
//             options={["Luxury Hotel", "Boutique Hotel", "Budget Hotel", "Resort", "Motel", "Hostel", "Serviced Apartment"]} />
//         </div>
//         <Textarea label="Description" value={form.description} onChange={v => upd("description", v)} placeholder="Describe the hotel..." rows={3} />
//         <div className="hm-g3">
//           <Select label="Star Rating" value={form.starRating} onChange={v => upd("starRating", v)} options={["1", "2", "3", "4", "5"]} />
//           <Input label="Year Built" type="number" value={form.yearBuilt} onChange={v => upd("yearBuilt", v)} placeholder="2010" />
//           <Input label="Price Per Night (₹)" type="number" value={form.pricePerNight} onChange={v => upd("pricePerNight", v)} placeholder="2500" />
//         </div>
//         <div className="hm-g3">
//           <Input label="Tax Percentage (%)" type="number" value={form.taxPercentage} onChange={v => upd("taxPercentage", v)} placeholder="12" />
//           <Input label="Service Charge (₹)" type="number" value={form.serviceCharge} onChange={v => upd("serviceCharge", v)} placeholder="200" />
//           <Input label="Extra Bed Charge (₹)" type="number" value={form.extraBedCharge} onChange={v => upd("extraBedCharge", v)} placeholder="500" />
//         </div>
//         <Input label="GST Number" value={form.gstNumber} onChange={v => upd("gstNumber", v)} placeholder="22AAAAA0000A1Z5" />

//         {/* CONTACT */}
//         <div className="hm-section-title">Contact Information</div>
//         <div className="hm-g2">
//           <Input label="Phone *" type="tel" value={form.phone} onChange={v => upd("phone", v)} placeholder="+91 98765 43210" />
//           <Input label="Alternate Phone" type="tel" value={form.alternatePhone} onChange={v => upd("alternatePhone", v)} placeholder="+91 98765 00000" />
//         </div>
//         <div className="hm-g2">
//           <Input label="Email" type="email" value={form.email} onChange={v => upd("email", v)} placeholder="info@hotel.com" />
//           <Input label="Website" type="url" value={form.website} onChange={v => upd("website", v)} placeholder="https://hotel.com" />
//         </div>

//         {/* LOCATION */}
//         <div className="hm-section-title">Location</div>
//         <div className="hm-g3">
//           <Input label="Country" value={form.country} onChange={v => upd("country", v)} placeholder="India" />
//           <Input label="State" value={form.state} onChange={v => upd("state", v)} placeholder="Rajasthan" />
//           <Input label="City" value={form.city} onChange={v => upd("city", v)} placeholder="Jaipur" />
//         </div>
//         <div className="hm-g3">
//           <Input label="Area" value={form.area} onChange={v => upd("area", v)} placeholder="MI Road" />
//           <Input label="Pincode" value={form.pincode} onChange={v => upd("pincode", v)} placeholder="302001" />
//           <Input label="Landmark" value={form.landmark} onChange={v => upd("landmark", v)} placeholder="Near City Palace" />
//         </div>
//         <Textarea label="Full Address" value={form.address} onChange={v => upd("address", v)} placeholder="Complete address..." rows={2} />
//         <div className="hm-g2">
//           <Input label="Latitude" type="number" value={form.latitude} onChange={v => upd("latitude", v)} placeholder="26.9124" />
//           <Input label="Longitude" type="number" value={form.longitude} onChange={v => upd("longitude", v)} placeholder="75.7873" />
//         </div>
//         <Input label="Virtual Tour Link" type="url" value={form.virtualTourLink} onChange={v => upd("virtualTourLink", v)} placeholder="https://tour.hotel.com" />

//         {/* VENDOR */}
//         <div className="hm-section-title">Vendor Details</div>
//         <div className="hm-g2">
//           <Input label="Vendor ID" value={form.vendorId} onChange={v => upd("vendorId", v)} placeholder="VENDOR001" />
//           <Input label="Vendor Name" value={form.vendorName} onChange={v => upd("vendorName", v)} placeholder="Rajesh Kumar" />
//         </div>

//         {/* TIMING */}
//         <div className="hm-section-title">Timing & Policies</div>
//         <div className="hm-g2">
//           <Input label="Check-In Time" type="time" value={form.checkInTime} onChange={v => upd("checkInTime", v)} />
//           <Input label="Check-Out Time" type="time" value={form.checkOutTime} onChange={v => upd("checkOutTime", v)} />
//         </div>
//         <div className="hm-g2">
//           <Textarea label="Cancellation Policy" value={form.cancellationPolicy} onChange={v => upd("cancellationPolicy", v)} placeholder="Free cancellation upto 24hrs..." rows={2} />
//           <Textarea label="Refund Policy" value={form.refundPolicy} onChange={v => upd("refundPolicy", v)} placeholder="Full refund within 7 days..." rows={2} />
//         </div>
//         <div className="hm-g2">
//           <Input label="Child Policy" value={form.childPolicy} onChange={v => upd("childPolicy", v)} placeholder="Children under 5 stay free" />
//           <Input label="Pet Policy" value={form.petPolicy} onChange={v => upd("petPolicy", v)} placeholder="No pets allowed" />
//         </div>
//         <CbGrid
//           items={[
...
//           ]}
//           checked={form}
//           onChange={(key, val) => upd(key, val)}
//         />

//         {/* PAYMENT */}
//         <div className="hm-section-title">Payment Methods</div>
//         <CbGrid items={PM_LIST} checked={form.paymentMethods} onChange={(key, val) => upd("paymentMethods", { ...form.paymentMethods, [key]: val })} />

//         {/* AMENITIES */}
//         <div className="hm-section-title">Amenities</div>
//         <CbGrid items={AMENITY_LIST} checked={form.amenities} onChange={(key, val) => upd("amenities", { ...form.amenities, [key]: val })} />

//         {/* HIGHLIGHTS */}
//         <div className="hm-section-title">Property Highlights</div>
//         <Input label="Comma separated" value={form.propertyHighlights} onChange={v => upd("propertyHighlights", v)} placeholder="Rooftop View, Heritage Property, Sea Facing" />

//         <div className="hm-section-title">Food & Dining</div>
//         <Input label="Comma separated" value={form.foodAndDining} onChange={v => upd("foodAndDining", v)} placeholder="Buffet Breakfast, Rooftop Dining, Multi-Cuisine" />

//         <div className="hm-section-title">Safety & Security</div>
//         <Input label="Comma separated" value={form.safetyAndSecurity} onChange={v => upd("safetyAndSecurity", v)} placeholder="CCTV, 24/7 Security, Fire Safety" />

//         <div className="hm-section-title">Business Facilities</div>
//         <Input label="Comma separated" value={form.businessFacilities} onChange={v => upd("businessFacilities", v)} placeholder="Conference Room, Business Center" />

//         <div className="hm-section-title">Transport Services</div>
//         <Input label="Comma separated" value={form.transportServices} onChange={v => upd("transportServices", v)} placeholder="Airport Pickup, Car Rental, Shuttle" />

//         {/* MEDIA */}
//         <div className="hm-section-title">Media & Images</div>
//         <div className="hm-g2">
//           <FG label="Hotel Images">
...
//             <span>Max 10 videos</span>
//           </div>
//         </FG>

//         {/* STATUS */}
//         <div className="hm-section-title">Status</div>
//         <Select label="Hotel Status" value={form.status} onChange={v => upd("status", v)}
//           options={[{ value: "pending", label: "Pending Review" }, { value: "approved", label: "Approved" }, { value: "rejected", label: "Rejected" }]} />

//         {/* ROOMS */}
//         <div className="hm-section-title">Room Configuration</div>
//         {form.rooms.map((room, i) => (
//           <RoomForm key={room.id} room={room} index={i} onChange={updateRoom} onRemove={removeRoom} />
//         ))}
//         <button type="button" className="hm-btn-add-room" onClick={addRoom}>
//           <Icon.Plus /> Add Room Type
//         </button>
//       </div>
//       <div className="hm-form-actions">
//         <button className="hm-btn-cancel" onClick={onClose}>Cancel</button>
//         <button className="hm-btn-submit" onClick={onSubmit}>{isEdit ? "Update Hotel" : "Save Hotel"}</button>
//       </div>
//     </>
//   );
// }
...
//   const val = v => v || <span style={{ color: "var(--muted)" }}>—</span>;
//   const Field = ({ label, children }) => (
//     <div className="hm-view-field"><label>{label}</label><span>{children}</span></div>
//   );
//   const Tags = ({ items }) => {
//     const arr = Array.isArray(items) ? items : (items ? items.split(",").map(s => s.trim()).filter(Boolean) : []);
//     return arr.length ? <div className="hm-tags">{arr.map((a, i) => <span key={i} className="hm-tag">{a}</span>)}</div>
//       : <span style={{ color: "var(--muted)", fontSize: 13 }}>—</span>;
//   };

//   const amenityLabels = AMENITY_LIST.filter(a => hotel.amenities?.[a.key]).map(a => a.label);
//   const pmLabels = PM_LIST.filter(p => hotel.paymentMethods?.[p.key]).map(p => p.label);
...
//           <button className="hm-close" onClick={onClose}><Icon.X /></button>
//         </div>
...
//               <Field label="Hotel Name">{val(hotel.hotelName)}</Field>
//               <Field label="Hotel Type">{val(hotel.hotelType)}</Field>
//               <Field label="Star Rating">{hotel.starRating ? "⭐".repeat(+hotel.starRating) : "—"}</Field>
//               <Field label="Year Built">{val(hotel.yearBuilt)}</Field>
//               <Field label="Price / Night">{hotel.pricePerNight ? `₹${Number(hotel.pricePerNight).toLocaleString()}` : "—"}</Field>
//               <Field label="Status"><span className={`hm-status hm-status-${hotel.status || "pending"}`}>{hotel.status || "pending"}</span></Field>
//               <Field label="Tax">{hotel.taxPercentage ? `${hotel.taxPercentage}%` : "—"}</Field>
//               <Field label="Service Charge">{hotel.serviceCharge ? `₹${hotel.serviceCharge}` : "—"}</Field>
//               <Field label="Extra Bed Charge">{hotel.extraBedCharge ? `₹${hotel.extraBedCharge}` : "—"}</Field>
//               <Field label="GST Number">{val(hotel.gstNumber)}</Field>
//               <Field label="Featured">{b(hotel.featured)}</Field>
//               <Field label="Active">{b(hotel.isActive)}</Field>
//             </div>
//             {hotel.description && <div className="hm-view-field" style={{ marginTop: 10 }}><label>Description</label><span>{hotel.description}</span></div>}
//           </div>

//           <div className="hm-view-section">
//             <h4>Contact Information</h4>
//             <div className="hm-view-grid">
//               <Field label="Phone">{val(hotel.phone)}</Field>
//               <Field label="Alternate Phone">{val(hotel.alternatePhone)}</Field>
//               <Field label="Email">{val(hotel.email)}</Field>
//               <Field label="Website">{val(hotel.website)}</Field>
//             </div>
//           </div>
...
//             <h4>Location</h4>
//             <div className="hm-view-grid">
//               <Field label="Country">{val(hotel.country)}</Field>
//               <Field label="State">{val(hotel.state)}</Field>
//               <Field label="City">{val(hotel.city)}</Field>
//               <Field label="Area">{val(hotel.area)}</Field>
//               <Field label="Pincode">{val(hotel.pincode)}</Field>
//               <Field label="Landmark">{val(hotel.landmark)}</Field>
//               <Field label="Latitude">{val(hotel.latitude)}</Field>
//               <Field label="Longitude">{val(hotel.longitude)}</Field>
//             </div>
//             {hotel.address && <div className="hm-view-field" style={{ marginTop: 10 }}><label>Full Address</label><span>{hotel.address}</span></div>}
//             {hotel.virtualTourLink && <div className="hm-view-field" style={{ marginTop: 8 }}><label>Virtual Tour</label><span><a href={hotel.virtualTourLink} target="_blank" style={{ color: "var(--accent)" }}>{hotel.virtualTourLink}</a></span></div>}
//           </div>

//           <div className="hm-view-section">
//             <h4>Vendor Details</h4>
//             <div className="hm-view-grid">
//               <Field label="Vendor ID">{val(hotel.vendorId)}</Field>
//               <Field label="Vendor Name">{val(hotel.vendorName)}</Field>
//             </div>
//           </div>
...
//             <div className="hm-view-grid">
//               <Field label="Check-In">{val(hotel.checkInTime)}</Field>
//               <Field label="Check-Out">{val(hotel.checkOutTime)}</Field>
//               <Field label="Early Check-In">{b(hotel.earlyCheckInAllowed)}</Field>
//               <Field label="Late Check-Out">{b(hotel.lateCheckOutAllowed)}</Field>
//               <Field label="Couple Friendly">{b(hotel.coupleFriendly)}</Field>
//               <Field label="Local ID Allowed">{b(hotel.localIdAllowed)}</Field>
//               <Field label="Child Policy">{val(hotel.childPolicy)}</Field>
//               <Field label="Pet Policy">{val(hotel.petPolicy)}</Field>
//             </div>
//             {hotel.cancellationPolicy && <div className="hm-view-field" style={{ marginTop: 8 }}><label>Cancellation Policy</label><span>{hotel.cancellationPolicy}</span></div>}
//             {hotel.refundPolicy && <div className="hm-view-field" style={{ marginTop: 8 }}><label>Refund Policy</label><span>{hotel.refundPolicy}</span></div>}
//           </div>

//           <div className="hm-view-section"><h4>Amenities</h4><Tags items={amenityLabels} /></div>
//           <div className="hm-view-section"><h4>Payment Methods</h4><Tags items={pmLabels} /></div>
//           <div className="hm-view-section"><h4>Property Highlights</h4><Tags items={hotel.propertyHighlights} /></div>
//           <div className="hm-view-section"><h4>Food & Dining</h4><Tags items={hotel.foodAndDining} /></div>
//           <div className="hm-view-section"><h4>Safety & Security</h4><Tags items={hotel.safetyAndSecurity} /></div>
//           <div className="hm-view-section"><h4>Business Facilities</h4><Tags items={hotel.businessFacilities} /></div>
//           <div className="hm-view-section"><h4>Transport Services</h4><Tags items={hotel.transportServices} /></div>
...
//                   <h5>{r.roomType || "Room"} #{ri + 1}{r.roomName ? ` — ${r.roomName}` : ""}</h5>
//                   <div className="hm-view-grid">
//                     <Field label="Room Type">{val(r.roomType)}</Field>
//                     <Field label="Base Price">{r.basePrice ? `₹${Number(r.basePrice).toLocaleString()}` : "—"}</Field>
//                     <Field label="Offer Price">{r.offerPrice ? `₹${Number(r.offerPrice).toLocaleString()}` : "—"}</Field>
//                     <Field label="Total Rooms">{val(r.totalRooms)}</Field>
//                     <Field label="Available">{val(r.availableRooms)}</Field>
//                     <Field label="Max Guests">{val(r.maxGuests)}</Field>
//                     <Field label="Adults">{val(r.adults)}</Field>
//                     <Field label="Children">{val(r.children)}</Field>
//                     <Field label="Room Size">{val(r.roomSize)}</Field>
//                     <Field label="Floor">{val(r.floorNumber)}</Field>
//                     <Field label="Bed Type">{val(r.bedType)}</Field>
//                     <Field label="Bed Count">{val(r.bedCount)}</Field>
//                     <Field label="View Type">{val(r.viewType)}</Field>
//                     <Field label="Bathroom">{val(r.bathroomType)}</Field>
//                     <Field label="Status"><span className={`hm-status ${r.status === "soldout" ? "hm-status-rejected" : "hm-status-approved"}`}>{r.status || "available"}</span></Field>
//                     <Field label="Tax">{r.tax ? `${r.tax}%` : "—"}</Field>
//                     <Field label="Extra Bed Charge">{r.extraBedCharge ? `₹${r.extraBedCharge}` : "—"}</Field>
//                     <Field label="Refundable">{b(r.refundable)}</Field>
//                   </div>
//                   <div className="hm-view-grid" style={{ marginTop: 10 }}>
//                     {ROOM_BOOL_FIELDS.map(({ key, label }) => (
//                       <Field key={key} label={label}>{b(r[key])}</Field>
//                     ))}
//                   </div>
//                   {r.cancellationPolicy && <div className="hm-view-field" style={{ marginTop: 8 }}><label>Cancellation</label><span>{r.cancellationPolicy}</span></div>}
//                 </div>
//               ))}
//             </div>
//           )}
...
//   // const [hotels, setHotels] = useState(() => {
//   //   try { return JSON.parse(localStorage.getItem("staypro_hotels") || "[]"); } catch { return []; }
//   // });
//   const [hotels, setHotels] = useState([]);
...
//   const [form, setForm] = useState(defaultHotel());
...
//     setHotels(list);
//     localStorage.setItem("staypro_hotels", JSON.stringify(list));
//   }, []);
...
//   const openAdd = () => { setForm(defaultHotel()); setEditIdx(null); setPanelOpen(true); };
//   const openEdit = i => { setForm({ ...hotels[i] }); setEditIdx(i); setPanelOpen(true); };
//   const closePanel = () => { setPanelOpen(false); setEditIdx(null); };

//   // const handleSubmit = () => {
//   //   if (!form.hotelName.trim()) { showToast("Hotel name is required!", "error"); return; }
//   //   const list = [...hotels];
//   //   if (editIdx !== null) { list[editIdx] = form; showToast("Hotel updated successfully!"); }
//   //   else { list.push(form); showToast("Hotel added successfully!"); }
//   //   save(list);
//   //   closePanel();
//   // };
...
// //   try {
// //     await axios.post("http://localhost:9000/api/hotels/add", form);
// //     alert("Hotel Added");
// //     fetchHotels();
// //     closePanel();
// //   } catch (err) {
...
//     // 🔥 API CALL
//     await axios.post(
//       "http://localhost:9000/api/hotels/add",
//       formData,
//       {
...
//   //   const list = hotels.filter((_, idx) => idx !== i);
//   //   save(list);
...
//  await axios.delete(`http://localhost:9000/api/hotels/delete/${hotel._id}`);
//  fetchHotels();
// };
...
//             <p>Manage all properties & listings</p>
//           </div>
//           <button className="hm-btn-add" onClick={openAdd}>
//             <Icon.Plus /> Add Hotel
//           </button>
//         </div>
...
//             <div className="lbl">Total Hotels</div>
//             <div className="val">{stats.total}</div>
//             <div className="bdg bdg-blue">All Properties</div>
//           </div>
//           <div className="hm-stat">
//             <div className="lbl">Approved</div>
//             <div className="val">{stats.approved}</div>
//             <div className="bdg bdg-green">Active</div>
//           </div>
//           <div className="hm-stat">
//             <div className="lbl">Pending</div>
//             <div className="val">{stats.pending}</div>
//             <div className="bdg bdg-amber">Review</div>
//           </div>
//           <div className="hm-stat">
//             <div className="lbl">Rejected</div>
//             <div className="val">{stats.rejected}</div>
//             <div className="bdg bdg-red">Inactive</div>
//           </div>
//         </div>

//         {/* TABLE */}
//         <div className="hm-table-card">
//           <div className="hm-table-top">
//             <h3>All Hotels</h3>
//             <div className="hm-search">
//               <Icon.Search />
//               <input placeholder="Search hotel..." value={search} onChange={e => setSearch(e.target.value)} />
//             </div>
//           </div>
...
//                       </td>
//                       <td style={{ color: "var(--muted)" }}>{h.hotelType || "—"}</td>
//                       <td>
...
//                         </div>
//                       </td>
//                       <td>{h.pricePerNight ? `₹${Number(h.pricePerNight).toLocaleString()}` : "—"}</td>
//                       <td><span className={`hm-status hm-status-${h.status || "pending"}`}>{h.status || "pending"}</span></td>
//                       <td>
//                         <div className="hm-actions">
//                           <button className="hm-icon-btn view" title="View" onClick={() => setViewHotel(h)}><Icon.Eye /></button>
//                           <button className="hm-icon-btn edit" title="Edit" onClick={() => openEdit(origIdx)}><Icon.Edit /></button>
//                           <button className="hm-icon-btn del" title="Delete" onClick={() => handleDelete(origIdx)}><Icon.Trash /></button>
//                         </div>
//                       </td>
//                     </tr>
//                   );
...
//             <div>
//               <h3>{editIdx !== null ? "Edit Hotel" : "Add New Hotel"}</h3>
//               <p>{editIdx !== null ? `Editing: ${form.hotelName}` : "Fill in all hotel details below"}</p>
//             </div>
//             <button className="hm-close" onClick={closePanel}><Icon.X /></button>
//           </div>
//           {panelOpen && (
//             <HotelForm form={form} setForm={setForm} onSubmit={handleSubmit} onClose={closePanel} isEdit={editIdx !== null} />
//           )}
//         </div>
//       </div>
...
// ─── CONFIG ──────────────────────────────────────────────────────────────────
const BASE_URL = "http://localhost:9000/api/hotels"; // apna URL yahan lagao
