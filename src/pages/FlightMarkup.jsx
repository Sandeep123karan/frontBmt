...
function FlightMarkup() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (editId) {
      await axios.put(`/api/flight-markup/${editId}`, formData);
    } else {
      await axios.post("/api/flight-markup", formData);
    }
  ...
  const handleDelete = async (id) => {
    await axios.delete(`/api/flight-markup/${id}`);
    fetchMarkupData();
  };

  const toggleStatus = async (id) => {
    await axios.put(`/api/flight-markup/status/${id}`);
    fetchMarkupData();
  };

  return (
    <div className="markup-container">
      <h2>Flight Markup Management</h2>
      ...
      <form className="markup-form" onSubmit={handleSubmit}>
        {[
          ...
          <input
            ...
            required={field === "markupFor" || field === "markupType"}
          />
        ))}

        <button type="submit" className="save-btn">
          {editId ? "Update" : "Add Markup"}
        </button>
      </form>
